...
Recommended branch protection for this repo. CI produces exactly one status
check, the `quality` job from `.github/workflows/ci.yml`, so that is the check
to require on `main`.
...
Work in short-lived branches (`feature/...`, `fix/...`, `chore/...`) and merge
to `main` via PR. Prefer squash merges to keep history linear.
